...
void UsartDmaPrintf_(const char *format, ...)
{
    uint16_t len,a1111;
    va_list args;
    va_start(args, format);
    len = vsnprintf((char *)Usart_TxBuffer, sizeof(Usart_TxBuffer) , (char *)format, args);
	a1111=sizeof(Usart_TxBuffer);
	Usart_TxBuffer[len]=0xFF;
	Usart_TxBuffer[len+1]=0xFF;
	Usart_TxBuffer[len+2]=0xFF;
    va_end(args);
    HAL_UART_Transmit_DMA(&huart7, Usart_TxBuffer, len+3);
}
